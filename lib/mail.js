...
var X_MAILER_NAME = "Nodemailer",
    X_MAILER_VERSION = "0.1.13; +http://www.nodemailer.org";
...
/**
 * mail.tmpMailDir -> String
 * 
 * Temp dir where the emails will be saved before being piped into 
 * sendmail.
 */
exports.tmpMailDir = '/tmp'
...
EmailMessage.prototype.send = function(callback){
    ...
    if(this.debug){
        var str = X_MAILER_NAME+", "+X_MAILER_VERSION+": "+instanceId;
        console.log(str);
    ...
    if(exports.sendmail){
        ...
        var tmpFile = exports.tmpMailDir + '/mail' + instanceId;

        fs.writeFile(tmpFile, msg, function(error) {
		    if (error) {
		        callback && callback(error, null);
		    } else {
		        exec(path + ' -t < /tmp/mail' + instanceId, function(error, stdin, stdout) {
			        fs.unlink(tmpFile, function(unlinkError) {
				        if (unlinkError) {
					        console.log('Unable to delete ' + tmpFile + ': ' + error);
				        }
				    });
			        if (error) {
				        callback && callback(error, null);
